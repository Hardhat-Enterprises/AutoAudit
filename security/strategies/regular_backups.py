...
class RegularBackups(Strategy):
    ...
    def emit_hits(self, text: str, source_file: str = "") -> List[Dict]:
        ...
        if not run_ml2:
            ...
            # Fallback: never return empty for ML1 files
            if not rows:
                rows.append(
                    _row(
                        "ML1",
        ...
        # -------------------------
        # Fallback: never return empty for ML2 files
        # -------------------------
        if not ml2_rows:
            ml2_rows.append(
                _row(
        ...
        # -------------------------
        # Implied ML1 row for ML2 evidence
        # -------------------------
