...
| 1.2.2 | L1 | Ensure sign-in to shared mailboxes is blocked | Automated | Automated | `exchange.mailbox.mailboxes` | Implemented | Graph `User.Read.All`; candidates approximated (unlicensed mail-enabled members); `SignInBlocked` derived from `accountEnabled`; see collector `detection_limitations` |
