...
| 3.1.1 | Microsoft 365 audit log search enabled | `exchange.organization.admin_audit_log_config` | Exchange Online PowerShell | Full `Get-AdminAuditLogConfig` result | Extracted `unified_audit_log_ingestion_enabled` value | Full Exchange config = AUTO-DIRECT; extracted field = AUTO-DERIVED | Automated | `controls.md` maps 3.1.1 to `AuditDisabled = False`, while the collector documentation identifies this evidence specifically with CIS 6.1.1. Confirmed documentation mismatch: control 3.1.1 is implemented with `admin_audit_log_config` and `unified_audit_log_ingestion_enabled`, while `controls.md` still documents the 6.1.1 mapping. |
...
- tracking correction of the confirmed 3.1.1 / 6.1.1 control-to-collector documentation mismatch
