...
* **export "in PDF, Excel, or CSV formats"**. An evidence scan generates a
  PDF, falling back to DOCX or plain text when the converter is unavailable
  (`security/reports/report_service.py`); there is no Excel and no CSV.
...
FORBIDDEN = {
    ...
    "certification-claim": re.compile(
        ...
        r"(?:certified|certification|attestation|audited)\b",
        re.I,
    ),
}
...
def test_the_gate_would_catch_the_claims_it_was_written_for() -> None:
    ...
    samples = {
        ...
        "certification-claim-with-type": "We're SOC 2 Type II certified",
    }
    for label, sample in samples.items():
        assert FORBIDDEN[label.removesuffix("-with-type")].search(sample), label
